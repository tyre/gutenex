defmodule Gutenex.PDF.Builders.ImageBuilderTest do
  ...
  setup do
    ...
    image_x_object = {101, [{{:obj, 101, 30}, {:stream, {:dict, %{"Type" => {:name, "XObject"}, "Subtype" => {:name, "Image"}, "Width" => 96, "Height" => 96, "Filter" => {:name, "FlateDecode"}, "BitsPerComponent" => 8, "DecodeParms" => {:dict, %{"Predictor" => 15, "Colors" => 3, "BitsPerComponent" => 8, "Columns" => 96}}, "ColorSpace" => {:name, "DeviceRGB"}}}, image.content}}, []]}
  ...
  test "#add_image_summary with no images" do
    ...
    assert new_render_context.image_summary_reference == {:ptr, 101, 40}
  ...
  test "#add_image_summary with one image", %{image: image, image_x_object: x_object} do
    ...
    new_render_context = ImageBuilder.add_image_summary(%RenderContext{
      ...
      image_aliases: %{"Img4" => {:ptr, 101, 30}},
      current_index: 101,
      generation_number: 30})
    assert new_render_context.current_index == 102
    assert new_render_context.image_summary_reference == {:ptr, 102, 30}
    assert new_render_context.image_objects == [raw_x_object, {{:obj, 102, 30}, {:dict, %{"Img4" => {:ptr, 101, 30}}}}]
  end
end
